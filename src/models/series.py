...
class Series(Base):
    ...
    translations = relationship(
        "SeriesTranslation", back_populates="series", cascade="all, delete-orphan"
    )
    staff = relationship(
        "SeriesStaff", back_populates="series", cascade="all, delete-orphan"
    )
    groups = relationship(
        "SeriesGroup", back_populates="series", cascade="all, delete-orphan"
    )
    external_links = relationship(
        "ExternalLink", back_populates="series", cascade="all, delete-orphan"
    )
...
class SeriesTranslation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("series_id", "language", name="_series_lang_uc"),
    )
...
class SeriesRelation(Base):
    ...
    __table_args__ = (
        UniqueConstraint("source_id", "target_id", name="_rel_source_target_uc"),
    )


__all__ = ["Series", "SeriesTranslation", "SeriesRelation", "series_genres"]
