...
def list_series(
    ...
    if genre:
        query = query.join(modelsSeries.series_genres).join(modelsGenres.Genre).filter(
...
def get_series_full(id: int, db: Session = Depends(database.get_db)):
    serie = (
        ...
        .options(
            ...
            joinedload(modelsSeries.Series.groups).joinedload(modelsStructure.SeriesGroup.translations),
            joinedload(modelsSeries.Series.groups).joinedload(modelsStructure.SeriesGroup.chapters).joinedload(modelsStructure.Chapter.translations),
        )
...
def create_series(
    ...
    for trans in data.translations:
        db.add(modelsSeries.SeriesTranslation(
    ...
    if data.genres:
        for g in data.genres:
            genre = db.query(modelsGenres.Genre).filter(modelsGenres.Genre.slug == g.slug).first()
            if genre:
...
def update_series(
    ...
    db_series = db.query(modelsSeries.Series).filter(modelsSeries.Series.id == id).first()
    if not db_series:
        raise HTTPException(status_code=404, detail="Serie no encontrada")
...
def get_groups_by_series(series_id: int, db: Session = Depends(database.get_db)):
    if not db.query(modelsSeries.Series).filter(modelsSeries.Series.id == series_id).first():
        raise HTTPException(status_code=404, detail="Serie no encontrada")

    return db.query(modelsStructure.SeriesGroup).filter(modelsStructure.SeriesGroup.series_id == series_id).all()
...
def create_group(
    ...
    if getattr(group_data, "translations", None):
        for trans in group_data.translations:
            db.add(modelsStructure.GroupTranslation(
...
def get_chapters_by_group(series_id: int, group_id: int, db: Session = Depends(database.get_db)):
    db_group = db.query(modelsStructure.SeriesGroup).filter(
...
def create_chapter(
    ...
    if getattr(chapter_data, "translations", None):
        for trans in chapter_data.translations:
            db.add(modelsStructure.ChapterTranslation(
